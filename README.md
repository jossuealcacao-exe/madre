...
## Privacidad: lo que nunca viaja por la sala

Cada CLI corre con su propio contexto privado (instrucciones de su organización, la cuenta con la que está firmada) y puede confundirlo con contexto compartido y escribirlo en una respuesta. En `⚙ CONNECTIONS → PRIVACY` nombras los términos que no deben aparecer en la sala: una empresa, una marca, un dominio. MADRE los sustituye por un marcador antes de que lleguen al ledger, al archivista, a los demás agentes o al dataset, y cada respuesta que lo necesitó lo dice. `PURGE ROOM` hace lo mismo con lo que la sala ya guardó. Los términos se quedan en `config.json`; la sala solo registra cuántos.
