...
export const CONDITIONS = [
  ...
  // ---- capability assistance: how an agent could gain a scope it lacks ----
  {
    ...
    diagnosis: 'Gemini CLI 0.60 has no image-generation tool in headless mode, even though Google offers image models (gemini-2.5-flash-image). PULSE already knows this: in a plan, image steps are routed to an agent that can (Codex).',
    remedy: 'Two routes. Fastest: ask @codex, or let the orchestrator route the image step to it. Native: connect an MCP image server to Gemini CLI (`gemini mcp add`), which needs your own server and key; PULSE\'s isolated Gemini home ignores MCP servers on purpose, so it would land in PULSE as a module ("Gemini image MCP") rather than a switch. When a Gemini CLI release exposes image generation, the scope switch turns on by itself.',
    fixes: same(['# now: in a CREATE request, address @codex for the image', '# native route (outside PULSE):', 'gemini mcp add <name> <command-or-url>   # an MCP server exposing an image tool', 'gemini mcp list']),
  },
  {
    ...
    diagnosis: 'Claude Code has no image-generation tool; Claude models describe and reason about images, they do not render them.',
    remedy: 'Route image steps to @codex (automatic in plans). A native route exists through an MCP image server (`claude mcp add`), but PULSE runs Claude with MCP disabled for isolation, so that would become a PULSE module, not a switch.',
    fixes: same(['# now: address @codex for images, or let the orchestrator route the step', '# native route (outside PULSE):', 'claude mcp add <name> -- <command>', 'claude mcp list']),
  },
  {
    ...
    diagnosis: 'OpenCode exposes no image-generation tool; it depends on the provider model and MCP servers configured by the user.',
    remedy: 'Route image steps to @codex. Natively, an MCP image server in opencode.json would add a tool, but PULSE runs OpenCode with --pure and its own ephemeral config, so that would be a PULSE module.',
    fixes: same(['# now: address @codex for images', '# native route (outside PULSE): add an "mcp" entry in ~/.config/opencode/opencode.json']),
  },
  {
    id: 'scope-web',
    severity: 'informational',
