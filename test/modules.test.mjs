...
const fakeCtx = (config = {}, extra = {}) => {
  ...
  const events = [];
  return {
    ctx: { projectRoot: '/p', stateRoot: '/s', config, env: {}, agents: [], room: { setAsh() {}, capabilities: () => ({}) }, readConfig: async () => config, updateConfig: async (patch) => { writes.push(patch); }, record: async (type, payload) => { events.push({ type, payload }); }, services: { imageKey: async () => null, setImageModule() {}, ...extra } },
    writes, events,
  };
};
...
test('registry: seven modules in MODULES order, the compat layer answers with the same objects, and Ollama serves its routes', async () => {
  ...
  const listed = await listExtensions({ projectRoot: '/tmp', config: { modules: { ripley: { enabled: true }, ash: { enabled: false } } } });
  ...
  assert.equal(byId.ash.status.detail, 'off · agents answer at their own length');
  assert.equal(byId.ash.warning, undefined, 'Ash still warns about something it no longer does');
...
test('registry: the Ash switch needs no confirming, because nothing it does is lossy', async () => {
  ...
  const ash = moduleById('ash');
  // It used to ask for a confirmation because it rewrote what the human wrote and could change
  // the meaning. It no longer touches a word anyone wrote, so it is an ordinary switch.
  const on = await ash.toggle(ctx, {});
