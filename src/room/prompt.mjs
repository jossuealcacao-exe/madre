...
// net is cast wide on purpose and in both languages: missing it costs the human one more
// sentence, and sending it always costs every turn that was never about a module.
export function wantsModule(text) {
  ...
  return [
    ...
    { id: 'mcp', text:
      mcpServers.length
        ...
        : null },
    { id: 'sdk', text: lease && sdk && wantsModule(text) ? `Building a MADRE module: MADRE is modular and the human may ask you for a module (a slash command, a tool for the agents, an integration). Read the SDK guide at ${sdk.guide} and the complete example at ${sdk.example}, then write ONE file named <id>.module.mjs (default export a plain spec object) ${lease.scratchDir ? `in ${lease.outDir}/${lease.scratchDir}` : 'in the lease'}. Do not touch ~/.pulse/modules or .madre/modules: MADRE shows the human an INSTALL card and they decide. A module runs inside MADRE with the human's permissions, so keep it small, honest about what it reaches, and never store credentials.` : null },
