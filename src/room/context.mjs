...
export async function contextFor({ memory, priorEvents, messageId, text, contextMaxChars, recallShare, remember = () => {} }) {
  const full = buildConversationContext(priorEvents, { excludeMessageId: messageId, maxChars: contextMaxChars });
  ...
  const recent = buildConversationContext(priorEvents, { excludeMessageId: messageId, maxChars: contextMaxChars - recallBudget });
