...
- Sistemas: MADRE se declara para macOS y Linux, y en Windows dentro de WSL. La sala se abre igual en Windows y lo dice, en la terminal y en el puente, en vez de dejar que alguien lo descubra cuando un agente no arranca. Las partes que asumían un sistema tipo Unix dejaron de hacerlo: la detección reconoce las extensiones ejecutables de Windows, los instaladores se lanzan con su intérprete y el reinicio usa el de cada sistema.
