...
- Cada versión en una ficha ahora es una etiqueta y pertenece a algo con nombre: `MADRE 0.4.0` para los módulos que vienen con MADRE, el archivo o la versión declarada para los tuyos, y una etiqueta por cada cosa de fuera que el módulo maneja, con la versión encontrada en esta computadora: `git 2.54.0`, `ollama 0.30.11`, `@jossuealcala/ahp-plus · INSTALLS 1.4.1`. Que algo falte también es una etiqueta, en ámbar.
- Y detectando de verdad apareció una mentira vieja: PLAYWRIGHT decía `@playwright/mcp 11.16.0 found` en una computadora donde ese paquete nunca se instaló. Preguntaba `npx --no @playwright/mcp --version`, y npx, cuando el paquete no existe, contesta con la versión de npm y sale con código 0. Ahora la versión se lee del `package.json` del paquete —subiendo por `node_modules` desde el proyecto y luego en la raíz global—, sin ejecutar nada. En esta computadora PLAYWRIGHT ya dice lo que es: no está instalado.
